...
//        model.addAttribute("lessons",lessonService.findByChapId(exam.getLesson().getChapter().getId()));
    ...
    @GetMapping("/listExamSubject/{subjectId}")
public String listExamSubject(@RequestParam Optional<String> keyword,
                              @PathVariable int subjectId, Model model,
                              @PageableDefault(value = 10) Pageable pageable) {
    Page<Exam> exams;
    if(keyword.isPresent()){
        exams = examService.findAllByNameExamContainingAndBySubject(subjectId, keyword.get(), pageable);
        model.addAttribute("keyword", keyword.get());
    } else {
        exams = examService.findAllBySubjectId(subjectId, pageable);
    }
...
//    model.addAttribute("lesson", lessonService.findById(subjectId));
